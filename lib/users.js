...
module.exports.respondFriend = function  respondFriend(me, friend, responce, cb) {
  pg.connect(DATABASE, function (error, client, done) {
    ...
    if(!responce){
      ...
      var query='DELETE from friendship where friend1=$1 and friend2=$2'
    }
    if(responce){
      console.log('ég fer hér inn enn samt virkar þetta ekki, eyði enn út');
    var values= [friend, me, true];
    var query = 'UPDATE friendship set (friend1, friend2, status) = ($1, $2, $3) where (friend1, friend2)=($1, $2)'
    }
    client.query(query, values, function (err, result) {
      done();
      if (err) {
        console.log('fekk error: '+ err);
        return cb(error);
      } else {
        console.log('þetta er úrkoman' + result);
        return cb(null, true);
      }
    });
  });
...
module.exports.addFriend = function addFriend (me, friend, cb) {
  iFriend(me, friend, function (err, result1){
    ...
    client.query(query, values, function (err, result) {
      ...
      else return cb('Þú átt nú þegar vináttu með þessum vini');
    });
  });
}
  ...
  pg.connect(DATABASE, function (error, client, done) {
    ...
    client.query(query, values, function (err, result) {
      ...
      if (err) {
        ...
        console.log('í users' + result.rows[0]);
...
function createUserWithHashAndSalt (username, name, salt, hash, purl, address, email, phone, description, cb) {
  pg.connect(DATABASE, function (error, client, done) {
    if (error) {
      return cb(error);
    }

    var values = [username, name, salt, hash, purl, address, email, phone, description];
    var query = 'INSERT into users (username, name, salt, hash, purl, adress, email, phone, description) VALUES($1, $2, $3, $4, $5, $6, $7, $8, $9)';
    client.query(query, values, function (err, result) {
      done();
...
module.exports.createUser = function createUser (username, name, password, purl, address, email, phone, description, cb) {
  hash(password, function (err, salt, hash) {
    if (err) {
      return cb(err);
    }

    createUserWithHashAndSalt(username, name, salt, hash, purl, address, email, phone, description, cb);
  });
};
