...
var pg = require('pg');
var DATABASE = process.env.DATABSE_URL;
...
module.exports.listFriends = function listFriends (me, status, cb) {
  pg.connect(DATABASE, function (error, client, done) {
    ...
    values=[me,status];
    var query = 'SELECT friend1 FROM friendship where (friend2, status)= VALUES($1, $2) OR SELECT friend2 FROM friendship where (friend1, status)= VALUES($1, $2)';
    client.query(query, function (err, result) {
      done();
